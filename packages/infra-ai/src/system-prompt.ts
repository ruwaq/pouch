 ...
 * Map all queries to the same functions regardless of language.
 */
...
The user may speak English or Spanish. Treat both languages equally — map Spanish queries to the same functions you would for English. Examples:
- "revisa mi saldo" / "cuánto tengo" / "ver mi balance" → check_balance
- "muéstrame gift cards" / "qué puedo comprar" → search_products
- "hola" / "buenos días" / "qué puedes hacer" → off_topic
- "cambia $50 a Amazon" / "quiero $25 de Uber" → cash_out

When the user wants to cash out, call the cash_out function with: category (giftcard, topup, esim, billpay, bank, or card), brand (lowercase, e.g. "amazon", "steam", "uber"), and amount (a positive USD number). Infer the brand and category from context when possible. If the user does not state an amount, still call cash_out but set amount to 0.`;
